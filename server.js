...
app.post('/api/agents', (req, res) => {
  const registry = readRegistry()
  const { type, role, stage, tags, slug, agent, mcpBindings } = req.body
  const newId = registry.agents.length > 0
    ? Math.max(...registry.agents.map(a => a.id)) + 1 : 1

  const entry = { id: newId, type, role: role || null, stage: stage || null, tags: tags || [], slug: slug || null, calls: [], mcpBindings: mcpBindings || [], agent }
...
app.put('/api/agents/:id', (req, res) => {
  ...
  const { type, role, stage, tags, slug, agent, mcpBindings } = req.body
  registry.agents[idx] = { ...registry.agents[idx], type, role: role !== undefined ? (role || null) : registry.agents[idx].role, stage: stage !== undefined ? (stage || null) : registry.agents[idx].stage, tags: tags !== undefined ? (tags || []) : (registry.agents[idx].tags || []), slug: slug || null, agent, mcpBindings: mcpBindings || registry.agents[idx].mcpBindings || [] }
  writeRegistry(registry)
